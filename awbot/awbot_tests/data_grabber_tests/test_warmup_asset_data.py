...
def test_warmup_asset_data_no_existing_item(mock_logger, mock_dbops, mock_yahoo_finance_api):
    ...
    mock_logger.info.assert_any_call(
        "No records found for GOOGL bulk inserting all available data..."
    )
    ...
    mock_dbops.put_price_data_in_table.assert_called_once_with(
        mock_prices, bulk_insert=True, overwrite=True
    )
